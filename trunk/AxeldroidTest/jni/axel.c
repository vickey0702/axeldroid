	...
	for( i = 0; i < axel->conf->num_connections; i ++ )
	{
		if( !axel->conn[i].enabled && axel->conn[i].currentbyte < axel->conn[i].lastbyte )
		{
			...
			else
			{
				...
				{
					pthread_kill( *axel->conn[i].setup_thread,SIGUSR1);
					axel->conn[i].state = 0;
				}
			}
		}
...
void save_state( axel_t *axel )
{
  ...
  void thread_exit1_handler(int sig)
  { 
            pthread_exit(0);
  }
...
void *setup_thread( void *c )
{
  ...
  actions.sa_flags = 0; 
  actions.sa_handler = thread_exit1_handler;
...
static void axel_message( axel_t *axel, char *format, ... )
{
	message_t m[1];// = malloc( sizeof( message_t ) ), *n = axel->message;
	va_list params;
